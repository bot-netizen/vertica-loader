...
def build_fact_copy_sql(cfg, stream_name):

    """COPY ... FROM STDIN for one tar archive. Bad rows are diverted to the reject
    table; a STREAM NAME lets us read totals back from LOAD_STREAMS.

    REJECTMAX is the bad-record limit (cfg load.max_bad_records): the archive
    tolerates up to that many rejected rows and still commits; reaching the limit
    aborts the COPY, which rolls the whole archive back. """
    ...
    return (
        f"COPY {fq(cfg,'target_table')} FROM STDIN "
...
def load_one_archive(cfg, entry, stream_name):
    ...
    node that holds it (so parse work spreads across the cluster instead of
    funnelling through one node). Returns a result dict.

    wait() on tar|zcat and check its exit code BEFORE commit, and pipefail
    (in tar_stream_cmd) makes a tar failure surface even though zcat is last in the pipe.
    A corrupt/partial archive, a COPY error (e.g. REJECTMAX exceeded), or a lost connection
    all roll the whole data load back, no partial loads.
    """
    ...
    proc = subprocess.Popen(
        ssh_base(cfg, entry["ssh_host"]) + [tar_stream_cmd(entry["path"])],
        stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    try:
        log.info(build_fact_copy_sql(cfg, stream_name))
        cur.copy(build_fact_copy_sql(cfg, stream_name), proc.stdout)
        proc.stdout.close()
        rc = proc.wait()                                 # check BEFORE commit
        if rc != 0:                                      # corrupt / partial tar|zcat
            raise RuntimeError(
                f"tar|zcat rc={rc}: {proc.stderr.read().decode(errors='replace').strip()}")
        conn.commit()   # all-or-nothing: commit here
        ...
        conn.rollback()                                  # discard the whole archive
        try:
            proc.kill()
        except Exception:
            pass
...
#        archive_on = cfg["paths"].get("archive_loaded", False)
#        for r in results:
#            r["archived_name"] = ""
#            if archive_on and r["status"] != "FAILED":
#                try:
#                    r["archived_name"] = archive_file(cfg, r)
#                except Exception as e:
#                    log.error("archive failed for %s: %s", r["name"], e)
#        if not archive_on:
#            log.info("archive_loaded=false — leaving loaded files in place")
